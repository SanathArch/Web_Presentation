...
- `done: ` → Mark item done: remove from Active TODOs, append to `$VAULT/todos/Completed TODOs Archive.md` under a dated `## $PROJECT (YYYY-MM-DD)` heading (create the file if it doesn't exist)
...
Fallback: Read the note and extract wikilinks, then Grep for `[[ ` across the vault.
...
If `[project]` is specified, filter results to notes also tagged `project/ `:

```bash
obsidian vault=$VAULT_NAME search query="type: <type>" path="projects/<project>"  
```

Fallback: Grep for `type: ` across `$VAULT`.

#### lookup layer [project]
...
Fallback: Grep for `layer: ` across `$VAULT/projects/`.
...
Append to source (forward direction): Build the new list locally by appending `[[]]` to the current values, then set:
...
Initialize BFS: Start with ` ` at depth 0. Maintain a visited set and a queue.
